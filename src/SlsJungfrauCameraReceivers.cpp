                           ...
                           yat::SharedPtr<CameraReceivers>  in_detector_receivers)
{
    ...
    for(receiver_index = 0 ; receiver_index < m_receivers_info.size() ; receiver_index++)
    {
        ...
        yat::SharedPtr<slsReceiverUsers > receiver;
        receiver.reset(new slsReceiverUsers(argc, args, ret));
    ...
    for(receiver_index = 0 ; receiver_index < m_receivers_info.size() ; receiver_index++)
    {
        yat::SharedPtr<slsReceiverUsers > receiver = m_receivers_info[receiver_index].m_receiver; // alias
        ...
        DEB_TRACE() << "registering AcquisitionFinished()";
        receiver->registerCallBackAcquisitionFinished(AcquisitionFinished, user_data);
 ...
 * \param m_receiver_index receiver index
 ************************************************************************/
void CameraReceivers::getAcquisitionData(int in_receiver_index)
{
    DEB_MEMBER_FUNCT();
 ...
 * \return ignored
 ************************************************************************/
int CameraReceivers::StartAcq(char     * in_file_path ,
                              char     * in_file_name ,
 ...
 * \param in_user_data pointer to user data object
 ************************************************************************/
void CameraReceivers::AcquisitionFinished(uint64_t   in_frames_nb,
                                          void     * in_user_data)
{
 ...
 * \param in_user_data pointer to user data object
 ************************************************************************/
void CameraReceivers::GetData(uint64_t   in_frame_number  ,
                              ...
                              void     * in_user_data     )
{
    ...
    CameraReceiverUserData * user_data = static_cast<CameraReceiverUserData *>(in_user_data);
    user_data->m_receivers->getAcquisitionData(user_data->m_receiver_index);
}
...
void CameraReceivers::createReceiversInfo(const std::string & in_config_file_name)
{
    ...
    while (std::getline(file, line))
    {
		...
		if (re.match(line, full_match)) 
        {
		    ...
		    if (!match_list.empty())
            {
			    for (int receiver_index = 0 ; receiver_index < match_list.size(); receiver_index++)
                {
    ...
    else
    {
	    ...
	    for (int receiver_index = 0 ; receiver_index < m_receivers_info.size(); receiver_index++)
        {
