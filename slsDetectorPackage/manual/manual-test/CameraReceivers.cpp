 ...
 *  \brief  detector acquisition receivers controller class implementation 
...
void CameraReceivers::init(const std::string & in_config_file_name)
{
    ...
    for(receiver_index = 0 ; receiver_index < receivers.size() ; receiver_index++)
    {
        ...
        std::cout << "creating the receiver "
                    << "(" << receiver_index << ") "
                    << receivers[receiver_index].getHostName() << " - "
                    << "tcpip port (" << receivers[receiver_index].getTcpipPort() << ")" << std::endl;
        ...
        if(ret == slsReceiverDefs::FAIL)
        {
            ...
            MsgErr << "Receivers::init failed! " 
                   << "Could not create the sls receiver (" 
                   << receiver_index << ")"
                   << std::endl;

    		throw CameraException(MsgErr.str());
        }
        else
        {
            std::cout << "receiver created - version (" << receiver->getReceiverVersion() << ")" << std::endl;
    ...
    for(receiver_index = 0 ; receiver_index < m_nb_receivers ; receiver_index++)
    {
        ...
        std::cout << "registering the receiver " << "(" << receiver_index << ") callbacks:" << std::endl;
        ...
        std::cout << "registering finishedAcquisition()" << std::endl;
...
{
    std::cout << "CameraReceivers::startedAcquisition : receiver " << in_receiver_index << std::endl;
}
...
{
    std::cout << "CameraReceivers::finishedAcquisition : receiver " << in_receiver_index << ", nb frames " << in_frames_nb << std::endl;
}
                                      ...
                                      std::vector<CameraReceiver> & out_receivers      )
{
    ...
    //-------------------------------------------------------------------------
    Detector_ns::RegEx re;
    Detector_ns::RegEx::MatchListType match_list; // for multiple results
    Detector_ns::RegEx::FullMatchType full_match; // for single result

    // opening the config file in read only text mode
    std::ifstream file(in_config_file_name.c_str(), std::ifstream::in);
    std::string   line;

    std::cout << "createReceivers - starting to parse config file : " << in_config_file_name << std::endl;
    ...
    while (std::getline(file, line))
    {
		...
		if (re.match(line, full_match)) 
        {
		    ...
		    if (!match_list.empty())
            {
			    for (size_t receiver_index = 0 ; receiver_index < match_list.size(); receiver_index++)
                {
                    ...
                    std::cout << "hostname (" 
                                << receiver_index << "):" 
                                << out_receivers[receiver_index].getHostName() << std::endl;
                }
            ...
            {
    		    throw CameraException("createReceivers failed! Could not initialize the camera!");
            }
        }
	    ...
	    if (re.match(line, full_match)) 
        {
            ...
            std::cout << "config matching line found : " << line << std::endl;
            std::cout << "receiver index : " << index << std::endl;
	        ...
	        if (re.match(line, full_match)) 
            {
                ...
                std::cout << "receiver tcpip port : " << tcpip_port << std::endl;
    ...
    {
	    throw CameraException("readConfigurationFile failed! Please set correctly the hostname.");
    }
    else
    {
        // logging the result
        ...
        {
            std::cout << "hostname ("   << out_receivers[receiver_index].getHostName()  << ")"
                        << " - "
                        << "tcpip port (" << out_receivers[receiver_index].getTcpipPort() << ")" << std::endl; 
        }
    }

    std::cout << "createReceivers - end of parsing" << std::endl;
}
                                            ...
                                            const std::string & in_label) const
{
    ...
    {
        std::ostringstream MsgErr;
        MsgErr << "readConfigurationFile failed! " 
